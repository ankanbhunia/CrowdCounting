...
class FreeScale(object):
    ...
    def __call__(self, img, mask):
        return img.resize((self.size[1], self.size[0]), Image.BILINEAR), Image.fromarray(np.array(mask.resize((self.size[1], self.size[0]), Image.NEAREST))*((img.size[0] * img.size[1])/(self.size[0] * self.size[1])))
